...
Releases: bump `version` in package.json; when the change syncs to the public
repo, a `v<version>` tag is pushed automatically and the public repo's
`release.yml` publishes to npm via trusted publishing (OIDC, tokenless).
