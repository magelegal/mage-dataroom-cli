...
export async function promptHidden(query: string): Promise<string> {
  ...
  return new Promise<string>((resolve) => {
    ...
    // oxlint-disable-next-line mage/no-double-assertion -- Node's readline muting hook `_writeToOutput` is an undocumented internal with no public typing, so the Interface type must be asserted to reach it
